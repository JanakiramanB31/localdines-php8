...
// echo '1' . $controller->_get('action');
// echo '<pre>';
// print_r($tpl['pickup_status']);
// print_r($controller->_get('delivery'));
// print_r($tpl['delivery_status']);
// // print_r($_SESSION);
// echo '</pre>';
// echo '<pre>';
// var_dump($controller->_get); // Is this a method or object?
// var_dump(method_exists($controller, '_get'));
$is_ordering_available = false;
$order_type = isset($STORAGE['type']) ? strtolower($STORAGE['type']) : 'pickup';
$check_type = $order_type . "_status";

if (isset($tpl["$check_type"]) && $tpl["$check_type"]['available']) {
  $is_ordering_available = true;
}
// echo $is_ordering_available;
// echo '</pre>';
?>
...
<div id="pjFdCartWrapper_<?php echo $controller->_get->toString('index'); ?>" class="panel panel-default">
  ...
  <div class="panel-body pjFdPanelBody">
    ...
    if ($is_ordering_available && $tpl['cart_box']['cart'] !== false && count($tpl['cart_box']['cart']) > 0) {
    ...
    ?>
                  ...
                  <div class="alert alert-dismissible clearfix" role="alert">
                    <div class="row">
                      ...
                      <div class="col-lg-2 col-md-3 col-sm-5 col-xs-2 fdCartPriceCol">
                        <span><?php echo pjCurrency::formatPrice($product_price + $extra_price, ""); ?></span>
                    ...
                    ?><small><?php
                              echo stripslashes($extra['name']); ?> (<?php
                                                                      if (intval($extra['price'])) {
                                                                        echo pjCurrency::formatPrice($extra['price']); ?> x <?php }
                                                                                                                          echo $extra['qty']; ?>)</small><?php echo $i != count($item['extra_arr']) ? '<br/>' : null;
                                                                                                                                                          $i++;
                                                                                                                                                        }
                                                                                                                                                          ?>
                  </div>
          ...
          ?>
                <div class="row">
                  <div class="alert alert-dismissible clearfix" role="alert">
                    ...
                    foreach ($item['extra_arr'] as $extra_id => $extra) {
                      $price += $extra['price'] * $extra['qty'];
                    ?><small><?php echo stripslashes($extra['name']); ?> (<?php
                                                                          if (intval($extra['price'])) {
                                                                            echo pjCurrency::formatPrice($extra['price']); ?> x <?php }
                                                                                                                              echo $extra['qty']; ?>)</small><?php echo $i != count($item['extra_arr']) ? '<br/>' : null;
                                                                                                                                                              $i++;
            ...
            <div class="row" id="postCodeSec">
              <div class="form-group">
                <div class="col-md-6 col-sm-6 col-xs-12">
                  <strong><?php //__('front_promo_code'); 
                          ?></strong>
                </div>
                <div class="col-md-12 col-sm-12 col-xs-12">
                  ...
                  <div id="delivery-status" style="display:none; color: #a94442; margin-top; 10px;text-align: center;">Sorry! Currently no drivers are available.</div>
                </div>
              </div>
            </div>
            <div class="col-md-12 col-sm-12 col-xs-12 text-right">
      ...
      if (isset($tpl["$check_type"]) && !$tpl["$check_type"]['available']) { ?>
        <p class="text-center fdEmptyCart" id="emptyCart"><?php echo $tpl["$check_type"]['message']; ?></p>
      <?php } else {
      ?><p class="text-center fdEmptyCart" id="emptyCart"><?php __('front_empty_cart'); ?></p><?php
                                                                                            }
                                                                                          }
                                                                                              ?>
