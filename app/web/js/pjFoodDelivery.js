...
(function (window, undefined) {
  ...
  FoodDelivery.prototype = {
    ...
    init: function (opts) {
      ...
      this.$container.on("click.fd", ".fdBtnHome", function (e) {
        ...
        return false;
      }).on("click.fd", ".fdQtyCheck", function (e) {
        // if (e && e.preventDefault) {
        // 	e.preventDefault();
        // }
        var index = pjQ.$(this).attr('data-index'),
        ...
        .on("click.fd", ".fdOperator", function (e) {
          ...
          if (addable == true) {
            ...
            pjQ.$.post([self.opts.folder, "index.php?controller=pjFrontCart&action=pjActionAddProduct", "&session_id=", self.opts.session_id].join(""), $frm.serialize()).done(function (data) {
              ...
              self.loadCart.apply(self, ['', function () {
                ...
                pjQ.$('.fdQtyCheck').prop('checked', false);
                // if ($type != 'pickup') {
        ...
        }).on("click.fd", ".fdButtonCheckout", async function (e) {
          if (e && e.preventDefault) {
            e.preventDefault();
          }
          var type = pjQ.$("#orderTypeCounter").val();
          ...
          if (type == 'delivery') {
            try {
              var postcode_input = $("#cartInputPostCode");
              var submit = true;
              // const deliveryAvailable = await checkDeliveryStatus();
              const deliveryAvailable = true;
              if (deliveryAvailable) {
                console.log('called me');
                validatePostcode(postcode_input, logged, self.opts.session_id, type, submit, hasPostcode);
              } else {
                pjQ.$("#delivery-status").css("display", "block");
              }
            } catch (error) {
            ...
            // checkDeliveryStatus().then(result => {
            //   console.log('result', result);
            //   if (result) {
            //     pjQ.$("#delivery-status").css("display", "none");
            //     pjQ.$("#btn-checkout").addClass("fdButtonCheckout");
            //     pjQ.$("#btn-checkout").addClass("enabled");
            //   } else {
            //     pjQ.$("#delivery-status").css("display", "block");
    ...
    loadMain: function () {
      // console.log(this.opts.mainCatID);
      var self = this,
        index = this.opts.index,
        params = {
          "locale": this.opts.locale,
          ...
          "index": this.opts.index,
          "type": "",
          "menu_category": this.opts.menu_category
        };
    ...
    loadCart: function (type, callback) {
      ...
      pjQ.$.get([self.opts.folder, "index.php?controller=pjFrontPublic&action=pjActionCart", "&session_id=", self.opts.session_id, type].join(""), params).done(function (data) {
        ...
        if ($("#orderTypeCounter").val() === 'delivery') {
          checkDeliveryStatus().then(result => {
            console.log('result', result);
            if (result) {
              pjQ.$("#delivery-status").css("display", "none");
        ...
        console.log('1744:', $("#orderTypeCounter").val());
  ...
  function checkDeliveryStatus() {
    var driver_app_api_url = "https://ahaadosa.eposplus.uk/driver/public/api/v1/admin/delivery/status";
    return new Promise((resolve, reject) => {
      $.ajax({
        type: "GET",
        url: driver_app_api_url,
        success: function (response) {
          if (response.data.is_delivery_available) {
            resolve(true);
          } else {
            resolve(false);
          }
  ...
  function validatePostcode($this, islogged, session, type, submit, hasPostcode) {
    var postcode = $this.val().trim().toUpperCase();
      ...
      $.ajax({
        ...
        success: function (data) {
          if (data.status == 200 && data.result) {
            ...
            var $pc_outward = result.outcode; // This is equivalent to postcode_outward
            var $pc_valid;
              ...
              $.ajax({
                ...
                success: function (response) {
                  if (response.code == 100) {
                    ...
                    pjQ.$.post([
                      $controller_url,
                      "index.php?controller=pjFrontEnd&action=pjActionSetType",
                      "&session_id=", session,
                      "&type=", type,
                      "&postcode=", ''
        ...
        error: function (xhr, status, error) {
          ...
          if (xhr.status === 404) {
            ...
            $("#post_code").addClass("has-error");
            $("#postCodeErr").text("Error validating postcode. Please try again.");
          }
          $("#postCodeErr").css("display", "block");
          $("#postCodeInSession").val('');
          pjQ.$.post([
